...
export async function getJournalEntry(id) {
  try {
    ...
    // Format date strings on the server for hydration safety
    const { format } = await import("date-fns");
    const createdAtFormatted = format(new Date(entry.createdAt), "PPP");
    const updatedAtFormatted = format(new Date(entry.updatedAt), "PPP 'at' p");

    return { ...entry, createdAtFormatted, updatedAtFormatted };
  } catch (error) {
    throw new Error(error.message);
  }
}
